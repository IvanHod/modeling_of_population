...
class Main:
	...
	def calculate_prediction(self, pr_year=2005, write_xls=False):
		...
		for num in range(1, 101, 1):
			...
			next_children = number_children + female_factor * self.get_number_middle_female(for_prediction)
			new_data = {'0-4': {'male': next_children * fm['male'], 'female': next_children * fm['female']}}
			...
			for interval in sorted(self.factors_by_year, key=lambda k: int(k.split('-')[0])):
				...
				data_by_year[next_year].append(int(union_count_genders(new_data[next_interval])))
